...
import {
  ArrowLeft, Search, X, Loader2, Users, CreditCard,
  Repeat, AlertCircle, ChevronLeft, ChevronRight, Eye, Tag,
} from 'lucide-react';
...
export default function PlanRegistrationsPage() {
  ...
  const fetch_ = useCallback(async () => {
    ...
    setError('');
    try {
      const p = new URLSearchParams({ plan: id, page: String(page), limit: '20', sort });
      if (search)  p.set('search', search);
      if (status)  p.set('status', status);
      if (payment) p.set('paymentType', payment);

      const res  = await fetch(`/api/admin/registrations/by-plan?${p}`);
      const data = await res.json();
      if (data.success) {
        setRegs(data.data);
        setSummary(data.summary);
        setPagination(data.pagination);
        setPlanName(data.planName ?? id);
      } else {
  ...
  const statCards = [
    { label: 'Total Students', value: summary.total,       icon: Users,       color: '#D97706', bg: '#FFFBEB' },
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="flex items-center gap-2 text-xs mb-0.5" style={{ color: '#9CA3AF' }}>
            <Link href="/admin/plans" className="hover:underline">Plans</Link>
            <span>›</span>
            <span>{planName || id}</span>
          </div>
          <h1 className="text-xl font-black tracking-tight" style={{ color: '#1F2937' }}>
            {planName || id} — Students
          </h1>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {(search || status || payment) && (
            <button onClick={() => { setSearch(''); setStatus(''); setPayment(''); }}
              className="flex items-center gap-1.5 px-3 py-2.5 text-sm rounded-xl"
      ...
      <div className="bg-white rounded-2xl overflow-hidden" style={{ border: '1px solid #E7DCCB', boxShadow: '0 2px 8px rgba(215,119,6,0.05)' }}>
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="font-bold text-sm" style={{ color: '#6B7280' }}>No students on this plan</p>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ backgroundColor: '#FFFAF3', borderBottom: '1px solid #E7DCCB' }}>
                    {['Student','Parent / Email','Phone','Programs','Payment','Status','Pay Status','Registered'].map(h => (
                      <th key={h} className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color: '#9CA3AF' }}>{h}</th>
                    ))}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {r.programs.programs.slice(0, 2).map(p => (
                            <span key={p} className="px-2 py-0.5 rounded-full text-[11px] font-bold capitalize" style={{ backgroundColor: '#FEF3C7', color: '#D97706' }}>{p}</span>
                          ))}
                          {r.programs.programs.length > 2 && <span className="text-[11px]" style={{ color: '#9CA3AF' }}>+{r.programs.programs.length - 2}</span>}
                        </div>
                      </td>
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="p-5 space-y-3 text-sm">
                {[
                  ...
                  ['Programs',   selected.programs.programs.join(', ')],
                  ...
                  <div key={label} className="flex justify-between gap-4">
                    ...
                    <span className="font-medium text-right capitalize" style={{ color: '#1F2937' }}>{value}</span>
                  </div>
                ))}
                ...
                <div className="flex justify-between gap-4 items-center">
                  <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#9CA3AF' }}>Pay Status</span>
